...
public class BricksTer extends Activity {
     ...
     * classes.
     */
	public static final String LOG_TAG = "BricksTer";
...
//    };
    ...
    public static final byte RED_FULL_REV = 'R';
    public static final byte RED_PWR_UP = 'A';
    public static final byte RED_PWR_DOWN = 'B';
    public static final byte RED_STOP = 'C';
    public static final byte RED_FLOAT = 'P';
    
    public static final byte BLUE_FULL_FWD = 'd';
    public static final byte BLUE_FULL_REV = 'h';
    public static final byte BLUE_PWR_UP = 'D';
    public static final byte BLUE_PWR_DOWN = 'H';
    public static final byte BLUE_STOP = 'L';
    public static final byte BLUE_FLOAT = '`';
    ...
    private int mCurrentGui = R.layout.momentary;
...
//        mEmulatorView.register(mKeyListener);
	...
	private void setupMomentaryGui() {
		...
		// Red reverse
		button = (Button) findViewById(R.id.button_red_rev);
		button.setOnTouchListener(new OnTouchListener() {
		...
		// Blue reverse
		button = (Button) findViewById(R.id.button_blue_rev);
		button.setOnTouchListener(new OnTouchListener() {
...
class TerminalEmulator {
    ...
    public void append(byte[] buffer, int base, int length) {
        for (int i = 0; i < length; i++) {
            ...
            try {
                if (BricksTer.LOG_CHARACTERS_FLAG) {
                    char printableB = (char) b;
                    if (b < 32 || b > 126) {
                        printableB = ' ';
                    }
                    Log.w(BricksTer.LOG_TAG, "'" + Character.toString(printableB)
                            + "' (" + Integer.toString(b) + ")");
                }
                process(b);
                mProcessedCharCount++;
            } catch (Exception e) {
                Log.e(BricksTer.LOG_TAG, "Exception while processing character "
    ...
    private void selectGraphicRendition() {
        for (int i = 0; i <= mArgIndex; i++) {
            ...
            if (code == 0) { // reset
                ...
                if (BricksTer.LOG_UNKNOWN_ESCAPE_SEQUENCES) {
                    Log.w(BricksTer.LOG_TAG, String.format("SGR unknown code %d", code));
                }
            }
        }
    }
...
*/
     ...
     */
    public void initialize(BricksTer bricksTer) {
    	mBricksTer = bricksTer;
        mTextSize = 15;
        mForeground = BricksTer.WHITE;
        mBackground = BricksTer.BLACK;
        updateText();
    ...
    void updateSize() {
        ...
        if (mKnownSize) {
            ...
            int h = visibleRect.height() - mBricksTer.getTitleHeight() - 2;
